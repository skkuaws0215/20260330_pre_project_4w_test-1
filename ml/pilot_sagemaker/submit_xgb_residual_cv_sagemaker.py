...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Submit SageMaker CV job for XGBoost+ResidualMLP.")
    ...
    p.add_argument(
        "--output-prefix",
        default=f"s3://{TEAM_BUCKET}/results/features_nextflow_team4/sagemaker/20260402/cv_xgb_residual",
    )
...
def _upload_source(pilot_dir: Path, code_bucket: str, region: str, ts: int) -> str:
    ...
    try:
        with tarfile.open(tar_path, "w:gz") as tar:
            for name in ("train_xgb_residual_cv_sagemaker.py", "run_residual_mlp_cv_local.py", "requirements.txt"):
                tar.add(pilot_dir / name, arcname=name)
            tar.add(
                pilot_dir.parents[1] / "results/features_nextflow_team4/fe_re_batch_runs/20260331/analysis_target_only/xgb_cv_tuning/xgb_best_params.json",
                arcname="results/features_nextflow_team4/fe_re_batch_runs/20260331/analysis_target_only/xgb_cv_tuning/xgb_best_params.json",
            )
        s3_uri = f"s3://{code_bucket}/team4-final-cv/source/cv-xgb-residual-{ts}.tar.gz"
...
def main() -> None:
    ...
    feat_dst = f"s3://{code_bucket}/team4-final-cv/data/xgb_residual/{ts}/features.parquet"
    lab_dst = f"s3://{code_bucket}/team4-final-cv/data/xgb_residual/{ts}/labels.parquet"
    ...
    est = PyTorch(
        entry_point="train_xgb_residual_cv_sagemaker.py",
        ...
        hyperparameters={"features_s3": feat_dst, "labels_s3": lab_dst, "seed": "42", "n_splits": "5"},
    )
    est.fit(wait=args.wait, job_name=f"team4-cv-xgb-residual-20260402-{ts}"[:63])
