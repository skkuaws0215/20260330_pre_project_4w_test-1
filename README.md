...
### B 파일럿: ML 4종 병렬 Training Job

- 입력: `abc_inputs/20260330_abc_v1/B/` (`features_b.parquet`, `labels.parquet`)
- 코드: `ml/pilot_sagemaker/train_tabular.py` (관리형 **PyTorch** 이미지를 Python 런타임으로만 사용)
- 제출: `python3 ml/pilot_sagemaker/submit_b_parallel.py`
- 산출 S3 prefix: `s3://drug-discovery-joe-raw-data-team4/results/features_nextflow_team4/sagemaker/b_pilot_4models/`
- 각 잡 종료 후 모델 아티팩트 tarball 안에 `metrics.json`, `artifact.joblib` 포함 (CloudWatch 로그에도 `[METRICS]` 출력)
