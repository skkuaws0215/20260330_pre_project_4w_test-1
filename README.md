...
- **XGBoost 제출 (기존):** `ml/pilot_sagemaker/submit_final_xgb_sagemaker.py` + 컨테이너 엔트리 `train_tabular.py`
- **ResidualMLP / GCN:** SageMaker 전용 학습 엔트리는 저장소에 **TBD** — 로컬 `run_residual_mlp_cv_local.py`, `run_graph_gnn_cv.py`와 동일 하이퍼·데이터 정책으로 맞출 것 (`final_model_summary.json`의 `submit_scripts` 참고).
...
- **SageMaker 통합 진행·Job 지표 채우기:** `sagemaker_experiment_dashboard_20260403.html` (**20260403_v2**) — 로컬 기준선 표 + <code>sagemaker_final_three</code> 번들(§2.5) + Training Job별 TBD 표.
