...
- **GitHub Pages (URL로 열기):** 저장소 **Settings → Pages → Build and deployment**에서 **Source: GitHub Actions**로 둔 뒤, 워크플루 `Deploy GitHub Pages`가 한 번 성공하면 사이트 주소는 보통 `https://<owner>.github.io/<repo>/` 이고, 대시보드는 `.../abc_smiles_experiment_dashboard.html` (루트 `index.html`에서 링크).
