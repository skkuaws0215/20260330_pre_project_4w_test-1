...
- **GitHub Pages (URL로 열기)**  
  - 주소 형식은 맞다: `https://skkuaws0215.github.io/20260330_pre_project_4w_test-1/abc_smiles_experiment_dashboard.html`  
  - **404면** 아직 “게시”가 안 된 것이다. 아래 둘 중 **하나만** 켜면 된다.  
  - **방법 A (가장 단순):** **Settings → Pages → Build and deployment → Source:** **Deploy from a branch** → Branch **main** / Folder **/ (root)** → Save. 1~2분 뒤 같은 URL 재시도. (루트에 `.nojekyll` 있음 → Jekyll이 HTML을 건너뜀.)  
  - **방법 B (Actions):** Source를 **GitHub Actions**로 두고, **Actions** 탭에서 `Deploy GitHub Pages` 워크플루가 **초록(성공)** 인지 확인. 첫 실행은 저장소에서 **Workflow permissions** 허용이 필요할 수 있다.  
  - 저장소가 **비공개(private)** 이면 무료 플랜에서 Pages가 제한되거나 404가 날 수 있다. 이 경우 **Public**으로 두거나 조직 정책을 확인한다.
