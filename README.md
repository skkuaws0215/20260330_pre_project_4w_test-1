...
- **다시 받을 수 있는 자료**는 저장소에 **굳이 올리지 않아도 됩니다.** 예: 웹 포털에서 내려받는 배치 결과(ADMETlab·SwissADME 등), `pip`/`conda`로 설치하는 패키지, 공개 URL의 원시 데이터. 대신 **어디서·어떤 명령으로 재현하는지**를 이 README, `nextflow/README.md`, 또는 해당 스크립트 주석/도움말에 적어 두면 다른 PC에서 `git clone` 후에도 같은 절차를 밟을 수 있습니다.
- **올리는 것이 좋은 것:** 코드, 대시보드 HTML, 팀이 합의한 **재현용 스냅샷**(중간 CSV/JSON 등) — “이 커밋 기준으로 보고서 수치가 이렇다”를 고정할 때 유리합니다.
- **올리지 말 것:** 비밀 키·토큰, 가상환경 폴더(`.venv`, `.venv_admet` 등), 용량이 큰 캐시만 있고 스크립트로 다시 만들 수 있는 산출물(필요 시 `.gitignore`로 제외).
