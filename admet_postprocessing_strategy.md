...
**입력 전제:** 전처리 산출에 ADMET 관련 데이터가 이미 포함됨 (예: `s3://.../results/admet/25_admet_results.parquet`). 이 경우 **예측 도구는 누락분 보완·검증**에 집중.
...
Nextflow FE는 주로 **범주 1 산출 보강**과 연결되고, **순위 리스트 + ADMET 조인 + 필터**는 위 도구 중 하나로 구현하면 됨.
